...
#
# Usage: 
# R CMD BATCH '--args  input1  input2  distance header1 header2 output'   HMM.DM.code/annotation.R
#
###########################################################################################################
...
###########################################################################################################
 ...
 procChr <- function(In, Ref){
      out<-matrix(NA, nrow=0, ncol=7); 
       len_in<-dim(In)[1];
         ...
         if (sum(!is.na(Ref))==0){
          out_1to4<-cbind(as.character(In[,1]), as.numeric(In[,2]) ,as.character(In[,9]),as.numeric(In[,8]), paste(as.character(In[,11]), as.character(In[,12]), sep=":"));
 ...
 # Read in files. Remove header line
